...
import { 
  ...
  FiFileText, FiMessageSquare, FiSliders, FiHome, FiTrendingUp, FiArchive, FiHelpCircle, FiSave
} from "react-icons/fi";
...
export default function AdminLayout() {
  ...
  const isHomepageSettingsPage = location.pathname === "/admin/homepage-settings";
  const isSiteSettingsPage = location.pathname === "/admin/site-settings";
  ...
  // Specific menu items for Aethelgard Subscribers page mockup
  const subscribersMenuItems = [
    { id: "manuscripts", label: "Manuscripts", path: "/admin/books", icon: <FiBook size={18} /> },
    { id: "analytics", label: "Analytics", path: "/admin", icon: <FiTrendingUp size={18} /> },
    { id: "community", label: "Community", path: "/admin/subscribers", icon: <FiUsers size={18} /> },
    { id: "research", label: "Research", path: "/admin/genres-tropes", icon: <FiSearch size={18} /> },
    { id: "archive", label: "Archive", path: "/admin/blogs", icon: <FiArchive size={18} /> },
  ];
  ...
  // Dark sidebar lists all items in mockup order for other pages (unified list for homepage/site settings)
  const unifiedDarkMenuItems = [
    { id: "dashboard", label: "Dashboard", path: "/admin", icon: <FiLayout size={18} /> },
    { id: "books", label: "Books", path: "/admin/books", icon: <FiBook size={18} /> },
    ...
    { id: "reviews", label: "Reviews", path: "/admin/reviews", icon: <FiMessageSquare size={18} /> },
    { id: "subscribers", label: "Subscribers", path: "/admin/subscribers", icon: <FiUsers size={18} /> },
    { id: "homepage-settings", label: "Homepage Settings", path: "/admin/homepage-settings", icon: <FiHome size={18} /> },
    { id: "site-settings", label: "Site Settings", path: "/admin/site-settings", icon: <FiSettings size={18} /> },
  ];

  // For backward compatibility on standard dark layout pages
  const darkMenuItems = [
    { id: "pen-names", label: "Pen Names", path: "/admin/pen-names", icon: <FiEdit3 size={18} /> },
    { id: "dashboard", label: "Dashboard", path: "/admin", icon: <FiLayout size={18} /> },
    { id: "books", label: "Books", path: "/admin/books", icon: <FiBook size={18} /> },
  ...
  const menuItems = isLightTheme 
    ...
    : isSubscribersPage
      ...
      : isHomepageSettingsPage || isSiteSettingsPage
        ? unifiedDarkMenuItems
        : isReviewsPage 
          ? reviewsMenuItems 
          : darkMenuItems;

  // Dynamic branding text based on mockup screenshots
  const brandName = isSubscribersPage
    ? "The Quill"
    : isHomepageSettingsPage
      ? "Mountain Library"
      : isSiteSettingsPage
        ? "Library Admin"
        : isGenresTropesPage 
    ...
    <div className="min-h-screen bg-[#FDFBF7] flex flex-col md:flex-row transition-colors duration-300 font-sans">
      ...
      <header className="md:hidden bg-[#0A180E] text-cream border-b border-gold/20 px-4 py-4 flex items-center justify-between z-30">
        <Link to="/admin" className="font-serif font-bold text-lg tracking-wide text-cream flex items-center gap-2">
          ...
          <span>{isSubscribersPage ? "Aethelgard" : brandName}</span>
        </Link>
        <button
      ...
      >
        <div className="space-y-6">
          ...
          <div className={`flex items-center gap-3 select-none py-2 border-b ${isLightTheme ? "border-[#E5E3DC]/40" : "border-white/10"}`}>
            {isSubscribersPage ? (
              /* Custom double-circle gold emblem enclosing a quill and open manuscript logo for Aethelgard */
              <div className="w-9 h-9 rounded-full border border-gold/40 flex items-center justify-center shrink-0 bg-gold/10">
          ...
          <nav className="space-y-1 text-left">
              ...
              <NavLink
                ...
                className={({ isActive }) => {
                  ...
                  if (isLightTheme) {
                    ...
                    // Dark theme sidebar
                    if (isGenresTropesPage || isReviewsPage || isSubscribersPage || isHomepageSettingsPage || isSiteSettingsPage) {
                      // Left border indicator for specific mockups
                      return `${baseClasses} rounded-r-lg border-l-4 ${
                        isActive
                          ? "bg-[#162A1D] text-[#C5A880] border-[#C5A880] font-bold"
            ...
            {/* Separator and Secondary Links for Dark Theme (Hidden on Reviews, Subscribers, Homepage, Site settings) */}
            {!isLightTheme && !isReviewsPage && !isSubscribersPage && !isHomepageSettingsPage && !isSiteSettingsPage && (
              <>
        ...
        {/* Bottom Panel - Hidden on Genres page, custom for Reviews and Subscribers */}
        {!isGenresTropesPage && (
          <div className={`border-t select-none space-y-4 pt-4 ${isLightTheme ? "border-[#E5E3DC]/40" : "border-white/10"}`}>
            ...
            {isSubscribersPage && (
              /* Special layout button for The Quill style */
              <button
                onClick={() => {
                  setSidebarOpen(false);
                  navigate("/admin/books/new");
                }}
                className="w-full flex items-center justify-center gap-2 py-3 bg-[#C5A880] hover:bg-[#A58860] text-[#0E1D14] rounded-lg text-xs font-bold uppercase tracking-wider shadow transition-all duration-150 cursor-pointer"
              >
                <span>+</span>
            ...
            {/* User Profile / Custom action links */}
            {isSubscribersPage ? (
              <div className="flex flex-col gap-2.5 text-left text-xs font-semibold uppercase tracking-wider text-[#E5E3DC]/70">
                <button
                  onClick={() => navigate("/admin/site-settings")}
                  className="flex items-center gap-3 px-4 py-2 hover:text-white transition-colors"
                >
                  <FiHelpCircle size={18} className="shrink-0" />
                  <span>Support</span>
                </button>
                <button
                  onClick={handleLogout}
                  className="flex items-center gap-3 px-4 py-2 hover:text-red-400 transition-colors"
                >
                  <FiLogOut size={18} className="shrink-0" />
                  <span>Logout</span>
                </button>
              ...
              <div className="text-left text-xs font-semibold uppercase tracking-wider text-[#E5E3DC]/70">
                <button
                  onClick={handleLogout}
                  className="flex items-center gap-3 px-4 py-2.5 hover:text-red-400 transition-colors cursor-pointer w-full text-left"
                >
                  <FiLogOut size={18} className="shrink-0" />
                  <span>Logout</span>
                </button>
              ...
              /* User Profile avatar box for others */
              <div className="flex items-center gap-3 text-left">
                <div className="w-9 h-9 rounded-full overflow-hidden border border-[#C5A880]/30 bg-[#C5A880]/15 flex items-center justify-center shrink-0">
                  {isLightTheme ? (
                    ...
                    /* Eleanor Vance portrait style */
                    <div className="w-full h-full bg-[#C5A880]/25 text-[#E5E3DC] flex items-center justify-center font-serif font-bold text-xs select-none">
                      ET
                    </div>
                  )}
                </div>
                <div className="text-left">
                  <span className={`block text-xs font-bold leading-tight ${isLightTheme ? "text-charcoal font-semibold" : "text-white"}`}>
                    {currentUser?.displayName || "Elias Thorne"}
                  </span>
      ...
      <div className="flex-grow flex flex-col min-h-screen relative overflow-hidden">
        ...
        {!isCustomHeaderPage && (
          <div className="hidden md:flex bg-white border-b border-[#E5E3DC] h-20 px-8 items-center justify-between z-20">
            ...
            {isSubscribersPage ? (
              /* Aethelgard custom header */
              <div className="w-full flex items-center justify-between">
                <div className="flex items-center gap-8">
                  <h1 className="font-serif font-bold text-3xl tracking-tight text-[#0E1D14] select-none">Aethelgard</h1>
                  <nav className="flex items-center gap-6 text-[11px] font-bold text-charcoal/50 uppercase tracking-widest">
                    <a href="#drafts" className="hover:text-[#0E1D14] transition-colors">Drafts</a>
                    <a href="#library" className="hover:text-[#0E1D14] transition-colors">Library</a>
                    <a href="#schedule" className="hover:text-[#0E1D14] transition-colors">Schedule</a>
                  </nav>
                </div>
                <div className="flex items-center gap-4">
                  {/* Search archives capsule */}
                  <div className="relative w-64">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-4 py-1.5 text-xs bg-[#F5F4F0] border border-[#E5E3DC]/60 rounded-full focus:outline-none focus:border-gold text-charcoal"
                    />
                  </div>
                  {/* Bell with red notification dot */}
                  <button className="text-charcoal/65 hover:text-[#C5A880] transition-colors relative" title="Notifications">
                    <FiBell size={16} />
                    <span className="absolute top-0.5 right-0.5 w-1.5 h-1.5 rounded-full bg-red-500" />
                  </button>
                  {/* Settings gear */}
                  <button className="text-charcoal/65 hover:text-[#C5A880] transition-colors" title="Settings">
                    <FiSettings size={16} />
                  </button>
                  {/* Publish button */}
                  <button 
                    onClick={() => alert("Publish operation triggered.")}
                    className="px-5 py-2 bg-[#0E1D14] hover:bg-[#1A3020] text-white font-sans font-bold text-[10px] uppercase tracking-wider rounded-full shadow transition-all duration-150"
                  >
                    Publish
                  </button>
                </div>
              ...
              /* Homepage settings custom header */
              <div className="w-full flex items-center justify-between">
                <h1 className="font-serif font-bold text-xl text-charcoal leading-none">Homepage Settings</h1>
                <div className="flex items-center gap-4">
                  <button className="text-charcoal/65 hover:text-[#C5A880] transition-colors" title="Notifications">
                    <FiBell size={16} />
                  </button>
                  <button 
                    form="homepage-settings-form"
                    type="submit"
                    className="px-5 py-2.5 bg-[#C5A880] hover:bg-[#A58860] text-[#0E1D14] font-sans font-bold text-xs uppercase tracking-wider rounded-lg shadow transition-all duration-150 flex items-center gap-2 cursor-pointer"
                  >
                    <FiSave size={14} />
              ...
              /* Site settings custom header */
              <div className="w-full flex items-center justify-between">
                {/* Search settings input on left */}
                <div className="relative w-64 text-left">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-4 py-2 text-xs bg-[#F5F4F0] border border-[#E5E3DC]/60 rounded-full focus:outline-none focus:border-gold text-charcoal"
                  />
                </div>
                <div className="flex items-center gap-6">
                  <h1 className="font-serif font-bold text-lg text-charcoal">Site Settings</h1>
                  <button 
                  ...
                  {/* Bell and user profile */}
                  <button className="text-charcoal/65 hover:text-[#C5A880] transition-colors" title="Notifications">
                    <FiBell size={16} />
                  </button>
                  <div className="w-9 h-9 rounded-full bg-[#FAF6EE] border border-[#E5E3DC] flex items-center justify-center font-bold text-xs text-[#0A180E] select-none overflow-hidden shrink-0 shadow-sm">
                    <svg className="w-full h-full text-charcoal-light" viewBox="0 0 40 40" fill="none" stroke="currentColor" strokeWidth="1.5">
                      <path d="M20 21a6 6 0 1 0 0-12 6 6 0 0 0 0 12zM8 34c0-6 6-8 12-8s12 2 12 8" />
                    </svg>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="flex items-center gap-6">
                  ...
                  {/* Add Book Button - Hidden on Genres and Reviews pages */}
                  {!isGenresTropesPage && !isReviewsPage && (
                    <Link
                      to="/admin/books/new"
