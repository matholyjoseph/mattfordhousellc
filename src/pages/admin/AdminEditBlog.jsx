...
export default function AdminEditBlog() {
  ...
  // Blog inputs prefilled (simulating fetch by ID)
  const [title, setTitle] = useState("My Writing Process: From Idea to Finished Draft");
  const [slug, setSlug] = useState("my-writing-process");
  const [content, setContent] = useState(
    "Outlining is one of the most rewarding steps of writing. For some, staring at a blank screen can be terrifying, but structured planning turns that fear into actionable steps. In this essay, I pull back the curtain on my workflow...\n\nStart typing your masterpiece here. Use the toolbar above to style your narrative. Intellectual depth and quiet luxury await every paragraph."
  );

  // SEO details
  const [metaTitle, setMetaTitle] = useState("Writing Process and Creative Journey");
  const [metaDescription, setMetaDescription] = useState(
    "Explore author Elias Thorne's unique outline framework and writing workflow, from creative incubation to polishing the final draft."
  );

  // Sidebar controls
  const [category, setCategory] = useState("Behind the Scenes");
  const [connectedBook, setConnectedBook] = useState("Whispers in the Pines");
  const [tags, setTags] = useState(["Philosophy", "Writing"]);
  ...
  const handleUpdate = (e) => {
    e.preventDefault();
    alert(`Blog post "${title}" updated successfully! (Mock Action)`);
                ...
                <div className="space-y-2 text-left">
                  ...
                  <div className="bg-white border border-[#E5E3DC]/60 rounded-xl p-5 text-left space-y-2 shadow-sm min-h-[175px]">
                    <h4 className="font-serif font-bold text-[#8C5E28] hover:underline cursor-pointer text-[15px] leading-tight">
                      {metaTitle || "Writing Process and Creative Journey"} | Elias Thorne
                    </h4>
                    <div className="text-[11px] text-[#2E6B40] font-sans flex items-center gap-1 font-medium leading-none">
                      ...
                      <span>blog</span>
                      <span>›</span>
                      <span className="max-w-[120px] truncate">{slug || "my-writing-process"}</span>
                    </div>
                    <p className="text-[12px] text-charcoal/70 leading-relaxed font-sans font-light">
                      {metaDescription || "Explore author Elias Thorne's unique outline framework..."}
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-[#E5E3DC]/60 shadow-sm space-y-4">
              ...
              <div className="space-y-3 text-xs font-sans">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-charcoal">Published</span>
                </div>
                <div className="flex justify-between items-center">
